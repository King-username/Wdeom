...
class Code extends Base
{
    ...
    function vCode($num = 4, $size = 20, $width = 0, $height = 0) {
        ...
        //设置验证码字符集合
        $str = "123456789abcdefghijkmnpqrstuvwxyzABCDEFGHIJKLMNPQRSTUVW";
        //保存获取的验证码
        $code = '';
        //随机选取字符
        for ($i = 0; $i < $num; $i++) {
        ...
        $name=time();
        //将图片转化为png格式
        imagepng($im, ROOT_PATH . '/public/uploads/code/'.$name.'.png');
        header("Content-type: image/png");
        imagedestroy($im);
        $vcode['code']=$code;
        $vcode['src']=ROOT_PATH .'/public/uploads/code/'.$name.'.png';
        unlink($vcode['src']);
        return $vcode;
    }
}